// Typed client helpers for the branch object-storage (buckets/objects) API.
//
...
// sites in `src/commands/buckets.ts` can switch over with no behavioural
// change.
...
// `.data`, so we expose that minimal shape and let the helpers return the
